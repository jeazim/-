...
public class Chang {
    ...
    public static Label label=new Label("选择查询字段");
    ...
    public static JComponent toFind()
    {
        ...
        //label.setText("人事变动记录表");
        //p1.add(label);
        p1.add(new JLabel("人事变动查询表"));
        ...
        jSelect.addActionListener(e->{
            ...
            switch (query1)
            {
                case "记录编号":sql+="personnel.num="+query2+" order by num asc;";break;
                case "员工号":sql+="id="+query2+" order by num asc;";break;
                case "变更":sql+="intro='"+query2+"' order by num asc;";break;
            }
            selectProcess(sql);
        });
       ...
       p4.setBorder(BorderFactory.createTitledBorder("新增人事变动"));
       JPanel p5=new JPanel(new FlowLayout(FlowLayout.LEFT,10,20));
        ...
        p5.add(new JLabel("员工号:"));
       id=new JTextField(10);
       p5.add(id);
       p6.add(new JLabel("变动项目:"));
        ...
        jCBSelectQueryField2.addItemListener(event -> {
            switch (event.getStateChange()) {
                case ItemEvent.SELECTED:
                    SelectQueryFieldStr = (String) event.getItem();
                    System.out.println("选中：" + SelectQueryFieldStr);
        ...
        upData.addActionListener(e->{
            ...
            {
                case "职位":sql+=1+",'"+jSelectFild2.getText().trim()+"');";sql2+=" job='"+jSelectFild2.getText().trim()+"'";
                ...
                break;
                case "部门":sql+=2+",'部门"+jSelectFild2.getText().trim()+"');";sql2+=" department="+jSelectFild2.getText().trim()+"";break;
                case "辞退":sql+=3+",'辞退"+jSelectFild2.getText().trim()+"');";sql2+=" state='F' ";break;
            }
    ...
    private static void upDataDepertProcess(String sql2) {
        ...
        try{
            if (DbProcess.executeUpdate(sql2) < 1) {
                ...
                //JOptionPane.showMessageDialog(null,
                //"xiugai","成功",JOptionPane.INFORMATION_MESSAGE);
            }
